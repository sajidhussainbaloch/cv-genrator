...
function heuristicAnalyze(text: string) {
  ...
  if (wc < 100) { weak.push("CV is too short (under 100 words)"); suggestions.push("Expand your CV"); }
  if (wc > 800) { weak.push("CV is too long (over 800 words)"); suggestions.push("Keep it concise — aim for 400-600 words"); }
  if (foundSkills.length === 0) { weak.push("No recognizable technical skills found"); suggestions.push("List your technical skills with proficiency levels"); }
...
app.post("/api/cv/improve", async (req, res) => {
  ...
  try {
    const aiResult = await callAI(
      `Rewrite this CV to be ATS-friendly: 1. Strong action verbs 2. Better structure 3. Relevant keywords 4. Quantifiable achievements 5. Max 600 words\n\nOriginal CV:\n"""${text}"""\n\nReturn ONLY the rewritten CV text.`,
    ...
    const improved = [
      ...
      "- Process Optimization & Efficiency", "- Data-driven Decision Making",
...
app.post("/api/cv/suggest", async (req, res) => {
  ...
  if (!text) return res.status(400).json({ error: "CV text is required" });
  const aiResult = await callAI(
    `Analyze CV and return JSON with: suggestedRole (string), suggestedSkills (array of 5-8), suggestedLocations (array of 3-4 including user location and Remote)\n\nCV:\n"""${text.slice(0, 3000)}"""`,
...
async function generateSearchQuery(role: string, skills: string, location: string, cvText?: string): Promise<string> {
  ...
  if (cv && cv.length > 50) {
    const aiQuery = await callAI(
      `Generate a concise job search query (max 12 words) including the location "${locPart}". Extract target role from CV, include key skills. Return ONLY the query.\n\nCV:\n"""${cv.slice(0, 3000)}"""`,
...
function pickJobBoardLinks(role: string, location: string, count: number = 6) {
  ...
  return shuffleArray(JOB_BOARDS).slice(0, count).map((b: any) => ({ name: b.name, url: b.url(q, loc), color: b.color }));
}
...
function scoreJobByContent(job: any, skills: string, cvText: string): number {
  ...
  const skillScore = skillList.length > 0 ? (skillList.filter((s: string) => combined.includes(s)).length / skillList.length) * 40 : 20;
  const keywordScore = ["remote", "hybrid", "senior", "lead", "engineer", "developer"].filter((k) => combined.includes(k)).length * 5;
  if (cvText) {
...
function estimateSalary(title: string): string {
  ...
  if (t.includes("engineer")) return "$90k - $140k";
...
function generateMockJobs(role: string, skills: string, location: string): any[] {
  ...
  const locations = [location || "San Francisco, CA", "New York, NY", "Seattle, WA", location ? `${location} (Remote)` : "Remote (US)", "Remote (Global)"];
  return Array.from({ length: 6 }, (_, i) => {
    const matchScore = Math.min(95, 55 + Math.floor(Math.random() * 35));
    const company = companies[Math.floor(Math.random() * companies.length)];
    const loc = locations[Math.floor(Math.random() * locations.length)];
    const titles = [`${role}`, `Senior ${role}`, `${role} - ${company}`, `Lead ${role}`];
    ...
    return {
      ...
      description: `${company} is seeking a talented ${titles[i % titles.length]} to join the team.`,
...
app.post("/api/providers", async (req, res) => {
  ...
  const { data, error } = await supabaseAdmin.from("providers").insert({
    user_id: userId, name, base_url, api_key, model, temperature: temperature || 0.7, max_tokens: max_tokens || 4096, is_active: false,
  }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json({ success: true, data });
});
...
app.post("/api/providers/test", async (req, res) => {
  ...
  try {
    const url = (base_url.endsWith("/chat/completions") ? base_url : `${base_url.replace(/\/+$/, "")}/chat/completions`);
    const response = await fetch(url, {
      method: "POST",
      headers: { Authorization: `Bearer ${api_key}`, "Content-Type": "application/json" },
      body: JSON.stringify({ model, messages: [{ role: "user", content: "Reply with just: OK" }], temperature: 0.7, max_tokens: 10 }),
    });
    const data = await response.json();
    if (data.choices?.[0]?.message?.content) return res.json({ success: true });
    res.json({ error: "Unexpected response format — check your provider settings" });
  } catch (e: any) {
    res.json({ error: e.message || "Connection failed" });
  }
});
