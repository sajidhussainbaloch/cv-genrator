...
function extractJSON(text: string): any {
  ...
  if (firstB !== -1 && lastB > firstB) {
    try { return JSON.parse(text.substring(firstB, lastB + 1)); } catch {}
  }
  return null;
}
...
function heuristicAnalyze(text: string) {
  ...
  const missingSkills = skillsTaxonomy.filter(
  ...
  if (!lower.includes("experience") && !lower.includes("work history")) {
    weakAreas.push("No experience section found");
    suggestions.push("Add a detailed work experience section with your roles and responsibilities");
  } else if (!/\d+/.test(text)) {
    weakAreas.push("No quantified achievements in experience");
  ...
  if (!lower.includes("education") && !lower.includes("university") && !lower.includes("degree")) {
    weakAreas.push("No education section found");
    suggestions.push("Include your education background with degrees and institutions");
  }

  if (!lower.includes("project")) {
    weakAreas.push("No projects section found");
    suggestions.push("Add key projects with technologies used and your contributions");
  }

  if (text.split(/\s+/).length < 100) {
    weakAreas.push("CV is too short (under 100 words)");
    suggestions.push("Expand your CV with more details about your experience and skills");
  }

  if (text.split(/\s+/).length > 800) {
    weakAreas.push("CV is too long (over 800 words)");
    suggestions.push("Keep your CV concise — aim for 400-600 words");
  }

  if (foundSkills.length === 0) {
    weakAreas.push("No recognizable technical skills found");
    suggestions.push("List your technical skills clearly with proficiency levels");
  }

  if (foundSkills.length > 3) {
    strengths.push(`Strong technical skill set: ${foundSkills.slice(0, 5).join(", ")}`);
  }
  if (lower.includes("lead") || lower.includes("manag")) {
    strengths.push("Demonstrates leadership and management experience");
  }
  if (lower.includes("team")) {
    strengths.push("Shows teamwork and collaboration experience");
  }
  if (/\d+%|\d+x|\$\d+/.test(text)) {
    strengths.push("Uses quantifiable achievements (great for ATS)");
  }
  if (strengths.length === 0) {
    strengths.push("CV has been submitted for analysis");
  }
  if (weakAreas.length === 0) {
    strengths.push("Well-structured CV with all key sections");
  }

  const sectionScore = Math.max(0, 60 - weakAreas.length * 10);
  const skillScore = Math.min(30, foundSkills.length * 3);
  const strengthBonus = Math.min(10, strengths.length * 3);
  const atsScore = Math.min(100, Math.max(10, sectionScore + skillScore + strengthBonus));

  return { atsScore, missingSkills, weakAreas, suggestions, strengths };
}
...
async function searchWeb(query: string): Promise<any[]> {
  const settings = getSettings();
  const customUrl = settings.searxngUrl?.trim();
...
app.post("/api/auth/signup", async (req, res) => {
  ...
  res.json({
    success: true, user,
    session: access_token ? { access_token, refresh_token, user, expires_in: 3600, token_type: "bearer" } : null,
  });
});
...
app.post("/api/auth/login", async (req, res) => {
  ...
  res.json({
    success: true, user,
    session: access_token ? { access_token, refresh_token, user, expires_in, token_type } : null,
  });
});
...
app.post("/api/cv/analyze", async (req, res) => {
  ...
  try {
    const aiResult = await callAI(
      `Analyze this CV and return a JSON object with:
      - atsScore (0-100)
      - missingSkills (array of missing important skills)
      - weakAreas (array of weak points in the CV)
    ...
    res.json({ success: true, data: heuristic });
  } catch (err: any) {
    const heuristic = heuristicAnalyze(text);
    res.json({ success: true, data: heuristic });
  }
...
app.post("/api/cv/improve", async (req, res) => {
  ...
  try {
    const aiResult = await callAI(
      `Rewrite this CV to be ATS-friendly and more impactful:
      ...
      5. Keep it professional and concise (max 600 words)
    ...
    if (aiResult) {
      return res.json({ success: true, optimizedText: aiResult });
    }
    ...
    res.status(500).json({ error: "Failed to improve CV" });
  }
});
...
app.post("/api/cv/suggest", async (req, res) => {
  ...
  const aiResult = await callAI(
    `Analyze this CV and return a JSON object with ONLY these fields:
    - suggestedRole (string): the most likely job title/role for this person
    - suggestedSkills (array of strings): top 5-8 key skills from the CV
    - suggestedLocations (array of strings): 3-4 suitable job locations including the user's current location if detectable, plus "Remote"
  ...
  if (aiResult) {
    const parsed = extractJSON(aiResult);
    if (parsed && parsed.suggestedRole) {
      return res.json({ success: true, data: parsed });
    }
  }

  // Fallback: extract from heuristic
  const lower = text.toLowerCase();
  const found = skillsTaxonomy.filter((s) => lower.includes(s.toLowerCase()));
  res.json({
    success: true,
    data: {
      suggestedRole: found[0] ? `${found[0]} Developer` : "Software Engineer",
      suggestedSkills: found.slice(0, 6),
      suggestedLocations: ["Remote", "United States", "United Kingdom", "Germany"],
    },
  });
});

async function generateSearchQuery(role: string, skills: string, location: string, cvText?: string): Promise<string> {
  const locPart = location?.trim() ? location.trim() : "Remote";

  if (cvText && cvText.trim().length > 50) {
    const aiQuery = await callAI(
      `Generate a concise job search query (max 12 words) including the location "${locPart}".
      - Extract the target job role from the CV
      - Include key skills mentioned
      - MUST include the location: ${locPart}
    ...
    if (aiQuery && aiQuery.trim()) return `${aiQuery.trim()} ${locPart}`.trim();
  }
...
function scoreJobByContent(job: any, skills: string, cvText: string): number {
  ...
  const skillMatches = skillList.filter((s) => combined.includes(s)).length;
  const skillScore = skillList.length > 0 ? (skillMatches / skillList.length) * 40 : 20;

  const recentKeywords = ["2025", "2026", "remote", "hybrid", "senior", "lead", "engineer", "developer"];
  const keywordScore = recentKeywords.filter((k) => combined.includes(k)).length * 5;

  if (cvText) {
    const cvLower = cvText.toLowerCase();
    const cvWords = new Set(cvLower.split(/\W+/).filter((w) => w.length > 3));
    const jobWords = new Set(combined.split(/\W+/).filter((w) => w.length > 3));
    let overlap = 0;
    cvWords.forEach((w) => { if (jobWords.has(w)) overlap++; });
    const relevanceScore = Math.min(30, (overlap / Math.max(cvWords.size, 1)) * 30);
    return Math.round(Math.min(98, skillScore + keywordScore + relevanceScore + 10));
  }
...
function extractCompany(result: any): string {
  ...
  if (result.parsedUrl?.host) {
    return result.parsedUrl.host.replace("www.", "").split(".")[0] || "Tech Company";
  }
  return "Tech Company";
}
...
function generateMatchReasons(skills: string, score: number, job?: any): string[] {
  const reasons: string[] = [
    ...
    "Experience level matches role",
    score > 80 ? "Strong overall fit" : "Relevant position",
  ];
  if (skills) {
    const skillList = skills.split(",").map((s) => s.trim()).filter(Boolean);
    if (skillList.length > 0) {
      reasons.push(`Matching skills: ${skillList.slice(0, 3).join(", ")}`);
    }
  }
  if (job?.title) {
    reasons.push(`Role: ${job.title}`);
  }
  return reasons;
}
...
function pickJobBoardLinks(role: string, location: string, count: number = 6): { name: string; url: string; color: string }[] {
  const q = encodeURIComponent(role || "software engineer");
  const loc = encodeURIComponent(location || "Remote");
  return shuffleArray(JOB_BOARDS).slice(0, count).map((board) => ({
    name: board.name,
    url: board.url(q, loc),
    color: board.color,
  }));
...
function generateMockJobs(role: string, skills: string, location: string): any[] {
  const companies = [
    ...
    "Spotify", "Shopify", "Stripe", "GitHub", "Atlassian", "Notion",
  ];
  const locations = [
    location || "San Francisco, CA",
    "New York, NY",
    "Seattle, WA",
    location ? `${location} (Remote)` : "Austin, TX",
    "Remote (US)",
    "Remote (Global)",
    "London, UK",
    "Berlin, Germany",
  ...
  return Array.from({ length: 6 }, (_, i) => {
    ...
    return {
      id: `job_mock_${Date.now()}_${i}`,
      title: titles[i % titles.length],
      company,
      location: loc,
      salary: estimateSalary(titles[i % titles.length]),
      description: `${company} is seeking a talented ${titles[i % titles.length]} to join our team. You will work on cutting-edge projects, collaborate with world-class engineers, and help shape the future of our platform.`,
      url: boardLinks[0].url,
      matchPercentage: matchScore,
      matchReasons: generateMatchReasons(skills, matchScore),
      isMock: true,
      boardLinks,
    };
  });
...
app.post("/api/jobs/search", async (req, res) => {
  ...
  try {
    ...
    const siteQueries = shuffleArray(JOB_BOARDS).slice(0, 3).map((b) => {
      return `site:${b.domain} ${searchQuery}`;
    });
    const jobBoardQueries = [...siteQueries, searchQuery];

    let jobResults: any[] = [];
    for (const q of jobBoardQueries) {
      jobResults = await searchWeb(q);
      if (jobResults.length > 0) break;
    }

    if (jobResults.length > 0) {
      const cvSummary = cvText ? await callAI(
      ...
      const matchedJobs = jobResults.slice(0, 12).map((result: any, idx: number) => {
        const score = scoreJobByContent(result, skills || "", cvText || "");
        return {
          id: `job_${Date.now()}_${idx}`,
          title: result.title || `${rol}`,
          company: extractCompany(result),
          location: result.metadata?.source || result.parsedUrl?.host || loc,
          salary: estimateSalary(result.title || ""),
          description: result.content || "",
          url: result.url || boardLinks[0]?.url || "#",
          matchPercentage: score,
          matchReasons: generateMatchReasons(skills || "", score, result),
          isMock: false,
          boardLinks,
        };
      });
...
async function supabaseData(path: string, token: string, options?: { method?: string; body?: any }) {
...
app.post("/api/cv/history", async (req, res) => {
  ...
  const result = await supabaseData("/rest/v1/cv_history", token, {
    method: "POST",
    body: { cv_text, filename, analysis, optimized_text, job_role, job_skills, job_location },
  });
  if (result.error) return res.status(500).json({ error: result.error });
...
app.delete("/api/cv/history/:id", async (req, res) => {
  ...
  if (!token) return res.status(401).json({ error: "No token" });
  const { id } = req.params;
  const result = await supabaseData(`/rest/v1/cv_history?id=eq.${id}`, token, { method: "DELETE" });
  if (result.error) return res.status(500).json({ error: result.error });
  res.json({ success: true });
});
...
function tryStartLocalSearXNG() {
  try {
    const proc = spawn("python", ["-m", "searxng", "--port", "8888", "--quiet"], {
      stdio: "ignore",
      detached: true,
    });
    proc.unref();
